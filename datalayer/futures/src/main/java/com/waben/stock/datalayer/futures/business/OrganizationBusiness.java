...
public class OrganizationBusiness {
	...
	public void futuresRatioSettlement(Long publisherId, Long benefitResourceId, Long futuresOrderId, String tradeNo,
		...
		try {
			Response<String> response = orgSettleInterface.futuresRatioSettlement(publisherId, benefitResourceId,
					futuresOrderId, tradeNo, totalQuantity, serviceFee, orderCloseFee, deferredFee);
			String code = response.getCode();
			if ("200".equals(code)) {
				return;
			} else if (ExceptionConstant.NETFLIX_CIRCUIT_EXCEPTION.equals(code)) {
